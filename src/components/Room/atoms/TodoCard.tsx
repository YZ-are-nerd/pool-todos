...
const TodoCard: React.FC<Props> = ({todo, changeWatcher}) => {
  ...
  if (renameMode) {
    ...
    <div className="w-full h-fit flex items-center gap-2">
      <div className="w-full h-fit flex items-center cursor-pointer gap-2 p-2 rounded-xl bg-neutral-800">
        <TextArea autofocus={true} onkeyup={saveNewName} placeholder='' setValue={setNewName} value={newName} /> 
      ...
      <button 
      onClick={() => newName === todoData.title ? setRenameMode(false) : saveNewName()} className='p-1 rounded-md bg-neutral-800'>
    ...
    <div onContextMenu={e => {e.preventDefault(); setEditMode(!editMode)}} className="w-full h-fit flex items-center gap-2">
      <div 
      onClick={() => setChecked(!checked)} className="w-full h-fit flex items-center cursor-pointer gap-2 p-2 rounded-xl bg-neutral-800 hover:bg-opacity-80">
